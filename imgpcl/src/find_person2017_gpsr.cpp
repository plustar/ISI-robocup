...
{
	//cout<<"findPersonCallback()\n";
    ROS_INFO("findPersonCallback received: %s\n", msg->data.c_str());
    if(msg->data == "person")   //接受到navigation的消息，开始检测
    {
        state = 0;
    }
}   

void locArriveCallback(const std_msgs::String::ConstPtr& msg)
{
	//cout<<"findPersonCallback()\n";
    ROS_INFO("locArriveCallback received: %s\n", msg->data.c_str());
    if(msg->data == "arrive_a" && if_arrive == -1)   //接受到navigation的消息，开始检测
    {
        if_arrive = 0;
    }
}  
...
{    
   ...
   // ros::Subscriber camInfo         = nh.subscribe("camera/rgb/camera_info", 1, camInfoCallback);
    ros::Subscriber nav_sub = nh.subscribe("/nav_output", 1, locArriveCallback);        //订阅navigation的消息
    ros::Subscriber voice_sub = nh.subscribe("/gpsr_object", 1, findPersonCallback);
    ros::spin();
