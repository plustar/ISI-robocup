...
void dstCallback(const std_msgs::String::ConstPtr& msg);
void srcCallback(const std_msgs::String::ConstPtr& msg);
...
{   	   
    ...
    ros::Subscriber srclocation_sub = nh_.subscribe("/gpsr_srclocation", 1, srcCallback);
    ros::Subscriber dstlocation_sub = nh_.subscribe("/gpsr_dstlocation", 1, dstCallback);
...
void callback(const ImageConstPtr& image, const PointCloud2::ConstPtr& Callback_Points)
{
    ...
    if(is_find)
    {
		...
		{ 
            ...
            str.assign (object_name[object_label]);
            std_msgs::String say;
            say.data = " i have arrived "+str + "location";    
            voice_pub.publish(say);                 
			state = 2;
		}
	#ifdef TIME_MODE         
...
void navCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(msg->data == "arriveb")   //接受到navigation的消息，开始检测
    {
        state = 0;
    }
}
...
{
	//cout<<"spCallback()\n";
    ROS_INFO("dstCallback received: %s\n", msg->data.c_str());
    if(msg->data != "")
    {
        ifdst_loc = 1;
    }
