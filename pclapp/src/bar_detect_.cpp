...
void turn_cb (const std_msgs::String& msg);
...
int main (int argc, char** argv)
{
  ...
  _enable = 0;
  //创建ROS的发布节点
  ros::Publisher pub = nh.advertise<std_msgs::String> ("/barplace", 1);
  // 为接受点云数据创建一个订阅节点
  ros::Subscriber image_sub = nh.subscribe ("/camera/depth/points", 1, cloud_cb);
  ros::Subscriber turn_sub = nh.subscribe ("/turnleft", 1, turn_cb);
...
void turn_cb (const std_msgs::String& msg)
{
  ++orient;
  _enable = 1;
  cout << "turn_cb times:"<<orient<<endl;
}
