...
int state =-1;// -1;                     //状态标记，-1  --  什么都不做
                                    ...
                                    //1 - 检测，调整位置中
                                    //2 - 检测到，并且已调节好
int object_label = 0;     //从模板中的第一个开始抓起,当前标签即为已抓取的物体数量
...
bool objectRec(IplImage *image, int detection_method, IplImage** temp, int temp_num, objectRecT& res)
{
    ...
    {
        if(sWindowHueDetect(image, temp[temp_num], object_recs[temp_num], similarity) )  //0.6 是相似度，可改（0是完全不相似，1是完全相似）
        {
            object_recs[temp_num].flag = 1;
        }
        else
        {
            object_recs[temp_num].flag = 0;
        }
    }
    else if(detection_method == 2)
    {
        //sidewindow+hue      
        if( sWindowHueDetectP(image, temp[temp_num], object_recs[temp_num], similarity) )
        {
            object_recs[temp_num].flag = 1;
        }
        else
	...
	if(object_recs[temp_num].flag == 1)
	{
	    det_obj_num++;
	    cvRectangle(image, cvPoint(object_recs[temp_num].rc.x, object_recs[temp_num].rc.y), cvPoint(object_recs[temp_num].rc.x+object_recs[temp_num].rc.width-1, object_recs[temp_num].rc.y+object_recs[temp_num].rc.height-1), CV_RGB(0, 255, 0), 1, 8, 0);
		ROS_INFO("recognized results:  object = %s, similarity = %f\n", object_name[temp_num], object_recs[temp_num].similarity);
		strcpy(text, object_name[temp_num]);
		cvPutText(image, text, cvPoint(object_recs[temp_num].rc.x, object_recs[temp_num].rc.y), &font, textColor);
	}
...
void callback(const ImageConstPtr& image, const PointCloud2::ConstPtr& Callback_Points)
{
    ...
    is_find = objectRec(imgColor, 1, tempImage, object_label, obj);
    ...
    if(is_find)
    {
        ...
        char saveimg[255];
        sprintf(saveimg, "%s%d.pdf", saveimg_filename, object_label);
...
void navCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(msg->data == "reached_table")   //接受到navigation的消息，开始检测
    {
        state = 0;
    }
    if(msg->data == "reached_cupboard")   //接受到navigation的消息，开始检测
    {
        state = -1; 
        geometry_msgs::Twist vel;
