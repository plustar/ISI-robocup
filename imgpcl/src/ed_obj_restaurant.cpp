...
{   	   
	...
	ifstream fin("/home/isi/2017_ws/src/imgpcl/template/ed_restaurant_list.txt");   //注意注意-------如果复制到其他地方，修改目录
    ...
    if(is_find)
    {
        ...
        //cout<<"dddddddddddddddddddddddddd\n";
   	   ...
   	   else
   	   {
   		imgpcl::pos newp;
		//ROS_INFO("P: %f %f %f\n", p.x, p.y, p.z);
		Two2Zero(p, newp, atan2(3.5, 15), -0.08,  -0.08, 0.08);  //atan(高度，长度)newp为物体相对于机械臂坐标系的坐标，最后三个参数是机械臂基坐标系x，y，z相对摄像机坐标系的转换 跟据结构变化而改变
		ROS_INFO("theta: %f \n", atan2(3.5, 15));
		ROS_INFO("p: %f %f %f\n", p.x, p.y, p.z);
		//newp.y = newp.y + 0.019;
...
void spCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(msg->data == "green_tea")
    {
        cout<<"received data :  green_tea\n";
        object_label = 0;
    }
    else if(msg->data == "cafe_au_lait")
    {
        cout<<"received data :  cafe_au_lait\n";
        object_label = 1;
    }
    else if(msg->data == "iced_tea")
    {
        cout<<"received data :  potato_stick\n";
        object_label = 2;
    }
    else if(msg->data == "grape_fruit_juice")
    {
        cout<<"received data :  green_tea\n";
        object_label = 3;
    }
    else if(msg->data == "strawbery_juice")
    {
        cout<<"received data :  potato_stick\n";
        object_label = 4;
    }
