...
void init()
{
	...
	image_sub = nh.subscribe<sensor_msgs::PointCloud2> ("/camera/depth_registered/points", 10, &DetectBar::cloud_Cb, this);
...
void cloud_Cb (const sensor_msgs::PointCloud2ConstPtr& input)  //特别注意的是这里面形参的数据格式
{
	...
	if((orient == 0||orient == 2)&&_enable == 1)
	{
		...
		double sum = 0;
		int ucount = cloud_filtered.size()/3;

		sort(cloud_filtered.data.begin(),cloud_filtered.data.end());//点云排序
	...
	if(orient == 3)
	{
		std_msgs::String str;
		cout<<"orient = = = = = = 3"<<endl;
		cout<<"min_dist[0]:"<<min_dist[0]<<endl;
		cout<<"min_dist[1]:"<<min_dist[1]<<endl;
