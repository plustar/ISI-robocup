...
{
    geometry_msgs::Twist vel;
    tf::Matrix3x3 mat(tf::Quaternion(0, 0, 0, theta)); 
    double yaw, pitch, roll;    
    mat.getEulerYPR(yaw, pitch, roll); 
    int count = 0;
    vel.angular.z = 0.2;
    float time = yaw/vel.angular.z;
    ros::Rate loop_rate(10);
    int num = time*10;
    //转90度
    //float theta = 5;
...
{   	   
	...
	ifstream fin("/home/isi/2017_ws/src/imgpcl/template/ed_final_list.txt");   //注意注意-------如果复制到其他地方，修改目录
    ...
    move_pub = nh_.advertise<geometry_msgs::Twist>("/mobile_base/commands/velocity",1);  //移动
    ...
    ros::Subscriber arm_sub = nh_.subscribe("/arm_state", 1, &armCallback,this);        //订阅arm的消息,确认是抓取状态还是松开状态
    ros::Subscriber nav_sub = nh_.subscribe("/nav2image", 1, &navCallback,this);        //订阅navigation的消息
    ros::Subscriber sp_sub = nh_.subscribe("/socket2topic_nd", 1, &naodianCallback,this);  //订阅voice的消息
...
void callback(const ImageConstPtr& image, const PointCloud2::ConstPtr& Callback_Points)
{
    ...
    if(is_find&&switch_case)
    {
		...
		if(newp.z>0.7) newp.z=0.7;//如果高度太高，说明高出桌面太多，加限制,根据桌面高度修改
...
void naodianCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(switch_case == 0)
    {
        if(msg->data == "f")
        {
            cout<<"received data :  forward\n";
            forward_robot(0.25);
        }
        else if(msg->data == "b")
        {
            cout<<"received data :  back\n";
            forward_robot(-0.25);
        }
        else if(msg->data == "l")
        {
            cout<<"received data :  turn left\n";
            turn_robot(2.5);
        }
        else if(msg->data == "r")
        {
            cout<<"received data :  turn right\n";
            turn_robot(-2.5);
        }
        else if(msg->data == "s")
        {
            cout<<"received data :  switch case\n";
            switch_case = 1;
        }
    }
    if(object_label == -1 && switch_case == 1)
    {
        if(msg->data == "a")
        {
        ...
        {
            cout<<"received data :  back to move\n"; 
            object_label = 2;
            switch_case = 0;
        }
    }
