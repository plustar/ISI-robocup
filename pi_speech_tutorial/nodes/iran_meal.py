...
class gpsr:
    ...
    def __init__(self):
        ...
        self.soundhandle.say("i am ready to setup the table for meal, would you like coffee mate or digestive", self.voice)
        rospy.sleep(20) 
        # Initiate Publishers
    ...
    def talkback(self, msg):
        ...
        print msg.data
        #else:
        if msg.data.find('coffee-mate') > -1:

            self.soundhandle.say('ok i will give you the coffee mate',self.voice)
        ...
        if msg.data.find('digestive') > -1:
            self.soundhandle.say('ok i will give the digestive',self.voice)
            rospy.sleep(5)
            self.gpsr_begin.publish('start')
            self.gpsr_srclocation.publish('table')
            self.gpsr_dstlocation.publish('sofa')
