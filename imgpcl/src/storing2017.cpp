...
void turn_robot(float theta,float time)
{
	...
	else
	{
		...
		if(theta2 > 0)
		{
			vel.angular.z = (theta2 > 0.8)?0.8:theta2;    
		}
		else
		{
			vel.angular.z = (theta2 > -0.8)?theta2:-0.8;
		}
...
{   	   
	...
	ifstream fin("/home/isi/2017_ws/src/imgpcl/template/storing_list.txt");   //注意注意-------如果复制到其他地方，修改目录
...
bool objectRec(IplImage *image, int detection_method, IplImage** temp, int temp_num, objectRecT& res)
{
	...
	for(int i=object_label; i<temp_num; i++)
	{
   	...
   	for(int i = object_label; i < temp_num; i++)
	{
		if(object_recs[i].flag == 1)
		{
			...
			//选择相似度最大的物体
    		if(sim < object_recs[i].similarity)
    		{
    ...
    if(is_find)
    {
        ...
        //cout<<"dddddddddddddddddddddddddd\n";
   	   ...
   	   else
   	   {
   		imgpcl::pos newp;
		//ROS_INFO("P: %f %f %f\n", p.x, p.y, p.z);
		Two2Zero(p, newp, atan2(0, 337.76), -0.09,  -0.255, 0.155);  //atan(高度，长度)newp为物体相对于机械臂坐标系的坐标，最后三个参数是机械臂基坐标系x，y，z相对摄像机坐标系的转换 跟据结构变化而改变
		ROS_INFO("theta: %f \n", atan2(0, 337.76));
		ROS_INFO("p: %f %f %f\n", p.x, p.y, p.z);
		//newp.y = newp.y + 0.019;
		...
		if(newp.z>0.3) newp.z=0.3;//如果高度太高，说明高出桌面太多，加限制,根据桌面高度修改
...
void armCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if(msg->data == "grasped")       //接受到arm的消息后，机器人撤退20cm
    {
		...
		state = -1; 
		forward_robot(-1.2,4.1);
		turn_robot(-3.1,5);

		//直行到cupboard//////////////////////
		forward_robot(0.4,15);

		//转90度
		//turn_robot(-3.1,5);
		//直行release
		//forward_robot(0.15*4.1,4.1);
    ...
    if(msg->data == "released")       //接受到arm的消息后，机器人撤退20cm
    {
		state = -1;
		//后退
		forward_robot(-0.15*15,15); 
		//转90度
		turn_robot(3.1,5);
		//直行到table//////////////////////
		forward_robot(0.15*14.5,14.5);
		//转90度
		turn_robot(3.05,5);
		state = 0;      
    }   
