...
{   	   
    ...
    img_numPub = nh_.advertise<std_msgs::UInt8>("det_img_num", 1);//检测到的物体数量
    
    ros::Subscriber arm_sub = nh_.subscribe("/armOK", 1, armCallback);        //订阅arm的消息
...
bool objectRec(IplImage *image, int detection_method, IplImage** temp, int temp_num, objectRecT& res)
{
	...
	for(int i=0; i<temp_num; i++)
	{
		...
		{
			//sidewindow+hue    
			cout<<"sidewindow+hue/n";  
			if( sWindowHueDetect(image, temp[i], object_recs[i], Similar) )  //0.6 是相似度，可改（0是完全不相似，1是完全相似）
			{
		...
		{
			//sidewindow+hue      
			if( sWindowHueDetectP(image, temp[i], object_recs[i], Similar) )
			{
				object_recs[i].flag = 1;
			}
			else
	...
	for(int i = 0; i < temp_num; i++)
	{
		if(object_recs[i].similarity >= Similar && object_recs[i].flag == 1)
		{
		    det_obj_num++;
		}
	}
	image_numP.data=det_obj_num;  
    ...
    if(is_find)
    {
        ...
        cvRectangle(imgColor, cvPoint(obj.rc.x, obj.rc.y), cvPoint(obj.rc.x+obj.rc.width-1, obj.rc.y+obj.rc.height-1), CV_RGB(0, 255, 0), 1, 8, 0);
   	   ...
   	   else
   	   {
			...
			if(newp.z>0.08)
				newp.z=0.08;//如果高度太高，说明高出桌面太多，加限制,根据桌面高度修改
...
bool adjustRobot(imgpcl::pos p, float x)
{
    ...
    //p.x p.y
    //在工作空间内
    if( sqrt(pow(p.x, 2) + pow(p.y, 2) + pow (p.z,2)) < 0.29 && sqrt(pow(p.x, 2) + pow(p.y, 2)) < 0.29 && sqrt(pow(p.x, 2) + pow(p.y, 2)) > 0.189/*0.2843*/ && p.y > 0.189&& p.y < 0.29 && abs(p.x)<0.05 )
    {   
        cout<<"start grasping\n";
        std_msgs::String say;
